    ...
    <section className="border-t border-navy/10 bg-offwhite px-6 py-24 md:px-10 md:py-32">
      <div className="mx-auto max-w-[1600px]">
        ...
        {clients.length === 0 ? (
          ...
          <div className="mt-10 grid grid-cols-2 gap-x-10 gap-y-12 sm:grid-cols-3 md:grid-cols-6">
            {clients.map((client) => {
              const logoUrl = mediaUrl(client.logo)
              return (
                <div
                  key={client.id}
                  className="flex h-14 items-center justify-center grayscale transition-all duration-300 hover:grayscale-0"
                >
                  {logoUrl && (
                    <Image
                      src={logoUrl}
